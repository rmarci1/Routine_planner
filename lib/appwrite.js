...
export const config = {
    ...
    accessoriesId: '675570e9003de660489b',
}
const {
    endpoint,
    platform,
...
export const updateAnythinginProfile = async (currentProfile,xp,coin,diamond) => {
    try{
...
export const getAccessories = async (currentProfile) => {
    try{
        const getAccessories = await databases.listDocuments(
            databaseId,
            accessoriesId,
        )
        const ids = getAccessories.documents.filter((doc) => doc.profile.map((profile) => profile.$id == currentProfile.$id));

        if(!getAccessories) throw Error;
        return ids;
    }   
    catch(error){
        throw new Error(error);
